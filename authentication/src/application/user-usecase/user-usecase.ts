...
class UserUsecaseApplication extends UserUsecase {
  getUser = async (): Promise<DetailedSecureUser> => {
  ...
  updateUser = async (payload: UpdateUser): Promise<DetailedSecureUser> => {
    validateUserUpdate(payload);

    const user = await this.userRepository.getUser();
    if (!user) throw NoUserCreatedYetError;

    const areCredentialsValid = await this.passwordService.comparePasswords(payload.password, user.password);
